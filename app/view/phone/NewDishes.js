Ext.define('Dishes.view.phone.NewDishes', {
    extend: 'Ext.Panel',
    ...
    config: {
        ...
        items: [{
            ...
            items: [{
				...
				id: 'disher',
                data: {},
                tpl: '<div style="cursor: pointer; height: 110px; text-align: center;">' +
                    '<tpl if=\"values.id == null\">' +
						'<span style="width: 100%; height: 80px; font-size: 80px; line-height: 80px; margin-top: 20px; text-align: center;" class="fa-stack">' +
						'<i style="color: #DDD;" class="fa fa-circle-thin fa-stack-1x"></i>' +
						'<i style="font-size: 30px; color: #DDD;" class="fa fa-hand-pointer-o fa-stack-1x" aria-hidden="true"></i>' +
						'</span>' +
                    '<tpl else>' +
						'<tpl if="values.picture == null"><span style="float: left; margin-left: 50px;  width: 70px; height: 70px; font-size: 70px; line-height: 70px;" class="fa-stack">' +
						'<i style="color: #333;" class="fa fa-circle fa-stack-1x"></i>' +
						'<i style="font-size: 35px; color: #EEE;" class="fa <tpl if=\"values.id == null\">fa-hand-pointer-o<tpl else>fa-user</tpl> fa-stack-1x"></i>' +
						'</span></tpl>' +
						'<tpl if="values.picture != null"><img style="border-radius: 50%;" src="ws.php?action=picture&employeeId={id}" width="70" height="70" /></tpl>' +
						'<div style="color: #333; height: 100%; font-size: 20px;"><b>{firstname:ellipsis(15)}</b> <span style="color: #887251;">{lastname:ellipsis(15)}</span></div>' +
                    '</tpl>' +
                    '</div>',
                listeners: {
                    tap: {
                        ...
                        fn: function() {
                                ...
                                this.popover = Ext.create('Ext.Panel', {
                                    ...
                                    items: {
                                        ...
                                        itemTpl: '<div style="cursor: pointer; height: 50px;">' +
                                            ...
                                            '<div style="color: #333; margin-left: 20px; float: left; height: 100%; font-size: 20px;"><b>{firstname:ellipsis(15)}</b><br/><span style="color: #887251;">{lastname:ellipsis(15)}</span></div></div>',
                                        listeners: {
                                            scope: this,
                                            select: function(l, r) {
                                                this.setData(r.data);
                            ...
                            this.popover.showBy(this, 'tl-cl');
                        }
                    }
                }
            },{
                width: 150,
                tpl: '<div style="width: 120px; margin-right: 10px; text-align: center;">' +
                    ...
                    '<div style="border: 3px solid #887251; border-top: 0; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px; line-height: 16px; padding: 5px; padding-top: 15px; font-size: 30px; color: #333;">' +
                    '<b>{date}</b><br/><span style="font-size: 10px;">{year}</span></div>' +
                    '</div>',
                data: {
			...
			itemTpl: '<div class="present-item" style="margin: 5px; cursor: pointer; width: 80px;">' +
				...
				items: [{
					...
					items: [{
						...
						items: [{
							...
							xtype: 'panel',
							id: 'couverts',
							tpl: '<span style="color: #333; font-size: 30px; padding-right: 10px;">{couverts}</span> <i style="font-size: 24px; color: #CCC;" class="fa fa-cutlery" aria-hidden="true"></i>',
							data: {couverts: 0},
							plus: function(p){
								var d = p.getData();
								d.couverts++;
						...
						items: [{
							...
							xtype: 'panel',
							id: 'boites',
							tpl: '<span style="color: #333; font-size: 30px; padding-right: 10px;">{boites}</span> <i style="font-size: 24px; color: #CCC;" class="fa fa-archive" aria-hidden="true"></i>',
							data: {boites: 0},
							plus: function(p){
								var d = p.getData();
								d.boites++;
						...
						items: [{
							...
							xtype: 'panel',
							id: 'cafes',
							tpl: '<span style="color: #333; font-size: 30px; padding-right: 10px;">{cafes}</span> <i style="font-size: 24px; color: #CCC;" class="fa fa-coffee" aria-hidden="true"></i>',
							data: {cafes: 0},
							plus: function(p){
								var d = p.getData();
								d.cafes++;
						...
						items: [{
							...
							xtype: 'panel',
							id: 'verres',
							tpl: '<span style="color: #333; font-size: 30px; padding-right: 10px;">{verres}</span> <i style="font-size: 24px; color: #CCC;" class="fa fa-glass" aria-hidden="true"></i>',
							data: {verres: 0},
							plus: function(p){
								var d = p.getData();
								d.verres++;
			...
			items: [{
				...
				flex: 1,
				iconCls: 'fa-check',
				style: 'background-color: #887251; color: #FFF;',
				handler: function(b){
					var disher = Ext.getCmp('disher'),
