Ext.define('Dishes.view.phone.Rules', {
    extend: 'Ext.Panel',
...
'</p>',
			...
			itemTpl: '<div style="cursor: pointer; margin: 5px; margin-right: 15px; height: 60px; border-bottom: 1px solid #EEE;">' +
						...
						'</span></tpl>' +
						'<tpl if="values.picture != null"><img style="margin-left: 10px; border-radius: 50%;" src="ws.php?action=picture&employeeId={id}" width="50" height="50" /></tpl>' +
						'<span style="display: inline-block; color: #333; margin-left: 10px; font-size: 20px;"><b>{firstname}</b> <span style="color: #887251;">{lastname}</span><br/>'+
						...
						'<tpl else><span style="font-size: 14px; color: #F46660;"><i class="fa fa-times" aria-hidden="true"></i> <i>Règlement non signé</i></span></tpl></span>' +
					'</div>',
			listeners: {
				itemtap: function(dv, index, target, rec, e, eOpts) {
					var pan = Ext.create('Ext.Panel', {
						...
						items: [{
							...
							xtype: 'panel',
							html: '<canvas width="300" height="200" id="signature"></canvas>',
							listeners: {
								painted: function(e){
									if (rec.get('signature') != null && rec.get('signature') != '') {
										this.setHtml('<img src="'+rec.get('signature')+'" width="300" height="200" />');
									} 
									else {
